...
3. Click **Agents** in the editor's category sidebar. The pane shows one card per supported agent — **Claude Code**, **Codex**, **Grok Build**, **Kimi Code**, and **Oh My Pi** — each with an enable switch. Claude Code is enabled and carries the orange **Primary** pill out of the box: the primary agent is the one that auto-launches in the session's first tab. Other enabled agents are installed and authenticated too, but you start them yourself from a new tab.

<p align="center">
  <img src="images/editor-agents.en.jpg" width="600" alt="The Agents pane of the workspace editor, showing the Claude Code card with its Primary pill, the four authentication modes, the Anthropic API key field, and the other agents' collapsed cards" />
</p>
...
6. That is all the Agents pane needs for a first session. (Enabling **Codex**, **Grok Build**, **Kimi Code**, or **Oh My Pi** alongside works the same way — each card gets its own auth mode — but one agent is plenty today.)
