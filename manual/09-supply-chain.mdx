...
- **Malware, typosquats, and rogue install scripts.** Packages built to be malicious from the start. The [socket.dev](18-glossary.mdx) and [Depi](18-glossary.mdx) providers flag or filter these, and install-script stripping removes the most common execution vector outright.

Enforcement is host-side by design. The proxy applies policy to the response before the agent sees it, so nothing running inside the VM — including a fully compromised agent — can loosen the rules. The in-VM `.npmrc` and `pip.conf` can only further restrict what the proxy already served, never widen it. Reputation-service API keys (socket.dev, Depi) are held on the host only and are never exported into the VM.
...
| Ecosystem | Intercepted hosts | Age gate | OSV | socket.dev | Depi | Script strip |
|---|---|---|---|---|---|---|
...
## Package filtering: socket.dev and Depi

The **Package filtering** group selects a reputation provider through a mutually exclusive radio: **None**, **socket.dev**, or **Depi**. The two providers work in fundamentally different ways — socket.dev is a pre-flight lookup the proxy consults before letting a fetch through; Depi replaces the npm registry outright. Only one can be active at a time by design (running both would double-filter every install), and selecting **None** disables both while keeping any stored keys. Whichever provider you choose, the other layers in this chapter — age gate, OSV, script stripping — still apply on top.
...
[Depi](18-glossary.mdx) is a drop-in secure npm registry by Lupin & Holmes (landh.tech) that serves pre-vetted packages. Instead of looking packages up, Bromure re-routes *every* npm registry request — metadata, tarballs, audit, anything addressed to `registry.npmjs.org` or `*.npmjs.org` — to Depi's npm-compatible filtering registry at `depi-npm-proxy.landh.tech:443`, attaching your key as an `Authorization: Bearer` header and stripping any Authorization header the guest sent. Depi rewrites tarball URLs in its packuments to point at itself, so those follow-up fetches get the key injected host-side too.

To enable it, select **Depi** in the radio and paste your key into the **API key:** secure field (**Get an API key** opens landh.tech). While the field is empty, an orange warning reads **Enter an API key — Depi stays off without one.** Each re-routed request is logged in the Security Log:

```
[depi] GET registry.npmjs.org/… → https://depi-npm-proxy.landh.tech
```

Error handling is explicit rather than silent:

- **401 (key rejected).** Bromure substitutes a clear plaintext error that npm prints ("Bromure: the Depi registry rejected the configured API key…", with the header `X-Bromure-Block: depi-auth`), logs it, and raises a one-shot GUI alert per workspace-and-key combination: "Depi rejected your API key". Headless SSH/TUI sessions do not get the GUI alert; they rely on the log line and the rewritten npm error.
- **403 (package blocked by Depi, or key not authorized for it).** Logged and passed through unchanged, so npm reports Depi's own refusal text.

Depi affects npm only — the other seven ecosystems are untouched by it. Depi replaces socket.dev in the radio, not your local policy: the age gate, OSV check, and script stripping still run on what Depi serves.
...
| **Package filtering** | **None** (no socket.dev or Depi key) |
...
- **Edits apply live.** Saving the workspace pushes the new policy to running sessions immediately — the proxy reads it per request, so no VM restart is ever needed. Each new or changed policy is confirmed with a one-line summary in the Security Log, for example:
  ...
  Misconfigurations are called out right in that summary: `socket.dev=key-set-but-no-toggle` means a key is entered but neither block is enabled, and `depi=selected-but-no-key` means Depi is chosen but off for lack of a key.
- **Where it is stored.** The policy lives in the workspace's `profile.json` under `~/Library/Application Support/BromureAC/profiles/<id>/` (only non-default fields are written). The socket.dev and Depi API keys are stored there too — host-side only, shown as secure fields in the UI, and never copied into the VM. There are no supply-chain-specific environment variables or launch arguments.
...
The pipeline's reputation checks depend on outbound calls from your Mac (never from the VM): `api.osv.dev` for OSV, `api.socket.dev` for socket.dev, `pypi.org` for the PyPI publish-time backstop, and `depi-npm-proxy.landh.tech` for Depi. When an enabled source cannot produce a verdict — network down after retries, HTTP error, rate-limiting, an auth failure, or an unsupported ecosystem — Bromure **fails closed** rather than silently allowing.
