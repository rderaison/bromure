...
This chapter is the complete guide to using it: opening it, driving it by hand, letting the agent drive it through the built-in [browser MCP server](18-glossary.mdx), keeping sign-ins, and the privacy model. It is the same pane introduced in [Sessions](06-sessions.mdx#the-agentic-browser-pane); everything here also applies when a remote agent drives it over a [rich-client](14-remote-access.mdx) connection.
...
> **Note:** On a [rich client](14-remote-access.mdx), a remote agent's browser-MCP stream can be spliced over the SSH connection so the remote agent drives the browser pane you are watching locally. One such relay is active at a time.
