---
title: "Remote Access & the Fat Client"
...
This chapter covers enabling remote access on the host, each of the three entry points, how the fat client mirrors a remote 1:1, reaching services inside remote VMs, exposing a dev server to the internet, and the security model that keeps all of it safe. Fleet-scale enrollment and cloud telemetry are a different system entirely — see [Enterprise](15-enterprise.mdx). The complete command reference lives in [Automation & the CLI](16-automation-cli.mdx).
...
On the first enable, the server generates an ed25519 host key. Its fingerprint — shown in the **Server host key** row and by `bromure-cli remote status`, in the form `256 SHA256:… bromure-ac-remote (ED25519)` — is what a connecting client verifies to be sure it reached the right Mac. Give this fingerprint to whoever connects (or check it yourself in the fat client's first-contact sheet) so a man-in-the-middle cannot impersonate the host. See [trust on first use](18-glossary.mdx) for how the fat client pins it.
...
## Mirroring a remote with the fat client

The fat client turns this Mac's copy of Bromure Agentic Coding into a live mirror of a remote one. You get a full window that renders the remote using the exact same sidebar, grid, and terminal views you already know — the same layout shown below for a local instance — but titled **Remote — `<name>`**, and every edit you make runs on the remote and echoes back.

<p align="center">
  <img src="images/workspaces.en.jpg" width="700" alt="The Bromure Agentic Coding workspace browser: a WORKSPACES sidebar with Grid and AUTOMATIONS sections, and a detail pane showing CPU, Memory, vCPUs, Disk, and Uptime cards above a CONFIGURATION summary. A fat-client mirror window renders a remote instance using this same layout." />
</p>
...
A few operations are **not** available from the fat client in this version:
...
Secrets never cross the link. When the fat client reads a remote profile to populate the editor, every secret field arrives **blanked**. On save, a blank field means "keep the stored value" and a typed one replaces it. API keys, tokens, and MCP environment secrets stay on the remote Mac. The fat client authenticates by SSH key alone once paired — there are no password prompts during mirroring.
...
### Multiple remotes (fleet subnet routing)

You can mirror several remotes at once; each opens its own window, SOCKS forwarder, and (optionally) tunnel. When two remotes advertise the same subnet — the near-certain `192.168.64.0/24` collision — the first is routed literally and the second is given a `100.64.<n>.0/24` CGNAT-space alias, with a 1:1 host-octet-preserving remap so both can be reached at once.
...
- **Off by default, one door.** No remote path exists until you enable the SSH server. Every path — human menu, CLI, fat client, port forwards, browser MCP — rides that one server and no other.
- **Encryption and transport.** All traffic is SSH. The fat client's control polling, terminal attaches, file transfers, port forwards, and browser-MCP relay all travel inside the SSH connection. Because swift-nio-ssh has no post-quantum key exchange, a modern OpenSSH client will print a "not using a post-quantum key exchange" warning; this is expected and does not indicate a downgrade attack.
...
- **What a session can do.** A human login can only reach the remote menu — never a shell — so it can create, configure, and drive workspaces, but it cannot run arbitrary host commands. The fat client's four machine verbs are similarly narrow: a control-socket bridge, a forward to a guest on the vmnet subnet, and a browser-MCP relay. Forwards to the host itself, the gateway, or the internet are refused.
...
- **Trust on first use.** The fat client pins the remote's host key per endpoint on first contact and warns loudly on any later change.
...
| The fat client keeps asking for a password | This Mac's key is not enrolled on the remote | Let one password login enroll it automatically, or run `bromure-cli remote key add '<this Mac's key>'` on the remote (**Show this Mac's key** copies it). |
