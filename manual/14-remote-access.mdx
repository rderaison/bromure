...
| **The rich client** | Another Mac running the app, or the iOS / iPadOS app | A live 1:1 [mirror window](#mirroring-a-remote-with-the-rich-client) of the remote instance, with bidirectional edits — reached [by address](#by-address-same-network) on a LAN or [through a bromure.io account](#through-a-bromureio-account-from-anywhere) from anywhere. |
...
| **My servers · bromure.io** is empty | Not signed in, or the server isn't advertising | Sign in (**Window → Enroll in bromure.io…**); on the remote, enable **Remote Access** so it heartbeats and appears. A greyed row means it stopped heartbeating. |
