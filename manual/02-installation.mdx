...
4. **Customizing the image.** The app downloads a small set of image tools published with the image (about 60 MB) and boots them in a throwaway helper VM, which runs the catalog's postinstall steps as root in a chroot on the image (see the next section). Progress reads "Customizing the image: waiting for the prompt…", "…: installing packages…", and so on. Nothing is fetched from third-party mirrors for this step; if the tools can't be downloaded, the app fetches equivalent ones from their upstream source instead.
...
The published image must be legally redistributable, so it contains strictly free software (Node.js, docker, kitty, gh, glab, kubectl, doctl, awscli, azure-cli, and more are baked in). Everything non-free is declared as signed [postinstall steps](18-glossary.mdx) in the catalog and installed on *your* machine, into *your* copy of the image. The baseline steps at version 4.6.0 are:
...
1. Downloads the image build tools (about 25 MB, cached in the support directory).
2. Allocates a fresh 24 GB sparse disk.
3. Boots a one-shot builder VM (4 vCPUs, 4 GB RAM), driven over its serial console, which debootstraps Ubuntu noble, installs the free tooling and GRUB.
4. Applies the same catalog postinstall steps as the download path.

Progress messages include "Building the image: downloading tools…", "Allocating 24GB sparse disk…", "Building the image: starting…", and "Building the image: installing Ubuntu and the tools…". The progress bar on this path is driven by installer log lines and caps at 97% until the final "Base image ready" jump.
...
| `https://dl-cdn.alpinelinux.org` | Image build tools — only for the local build, or when the image tools published with the image can't be downloaded |
...
| `~/Library/Application Support/BromureAC/provisioner-vmlinuz`, `provisioner-initrd` | Cached image tools (downloaded with the image) |
| `~/Library/Application Support/BromureAC/alpine-vmlinuz`, `alpine-initramfs`, `alpine-initramfs-shimmed` | Cached build tools (local build and fallback) |
...
The installed image records its version in `base.version`. The *major* part comes from the version constant bundled in the app (`200` at version 4.3.0, unchanged through 4.6.0); any rebuild, re-download, or amendment at the same major appends a [dot-revision](18-glossary.mdx) (`200` → `200.1` → `200.2`). The sidecar `image-state.json` records where the image came from and which postinstall step UUIDs have been applied.
