...
While the VM boots, an animated "dive" overlay covers the terminal area — the workspace name over a scanning progress bar, with an optional status line. It only appears if boot takes more than 0.4 seconds, and it fades out the moment the guest reports its first tab roster. A workspace still on the legacy shared-folder home is moved to the private ext4 image automatically on its next launch — a one-time migration that copies the home into the image and keeps your previous home as a backup; during it the overlay shows determinate progress instead (for example "MOVING HOME — 42% OF 7.9 GB").
...
| --- | --- | --- |
| Newer base image | The shared base image was rebuilt since this workspace's disk was cloned | **Yes, upgrade** (re-clone from the new base; home untouched) / **No** (launch as-is; not asked again until a different base ships) / **Remind me later** (launch as-is; asked again on the first boot after a day) |
