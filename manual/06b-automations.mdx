...
## The AUTOMATIONS sidebar section

Automations live in their own group in the main window's sidebar, below **WORKSPACES** and the pinned **Grid** node. The **AUTOMATIONS** header carries a **+** button (tooltip *New automation*); each automation is one row, tinted with its owning workspace's accent color.

<p align="center">
  <img src="images/automations.en.jpg" width="700" alt="The main window: a WORKSPACES sidebar section with the Grid node and workspaces, and below it an AUTOMATIONS section listing a “Review incoming PRs” automation summarized as “PR · acme/webapp”." />
</p>

Each row shows:

- The automation's **name** (or *Untitled automation*).
- A one-line **summary** of its trigger and next fire — for example *Weekdays 9:00 · next Fri 09:00*, *PR · owner/repo*, or *Chained*. A paused automation reads *Paused* instead.
- The **last run's outcome** as a small glyph on the trailing edge (see [Run history](#run-history-and-next-fire-times)).

Clicking a row opens its editor as a full-stage overlay, the same surface pattern as the [VM dashboard](06-sessions.mdx#the-vm-dashboard). Right-click a row for its context menu: **Run Now**, **Pause** / **Resume**, and **Delete…**.
...
Every fire is recorded, and the sidebar row shows the last outcome as a glyph:

| Outcome | Meaning | Glyph |
| --- | --- | --- |
| **launched** | The agent started in a fresh worktree. | check-circle |
| **skipped** | The fire was suppressed — the Mac was asleep, or the workspace was off with **Start the workspace if needed** disabled. | forward-end |
| **failed** | The run could not be launched (the workspace is gone, or it did not boot in time). | x-circle |
| **blocked** | An event item was stopped by the injection screen. | shield-slash |
