...
## Remote access and the fat client

The optional SSH remote-access front door and the fat-client mirror are covered in [Remote Access](14-remote-access.mdx). The symptoms below are the ones that block a connection.
...
**Cause.** A *headless* remote — the app is running but has no unified window open — cannot accept or serve grid-layout edits, though everything else mirrors. Some prompt-driven worktree operations (new worktree, merge, resolve) are also not available from the fat client in v1, and multi-host subnet aliasing is a documented design limitation, not a shipped behavior: a single remote host is the supported configuration.

**Fix.** Open a session window on the remote so it has a unified window to mirror. For worktree create/merge/resolve on a remote, use the SSH/TUI remote menu instead of the fat-client toolbar.

### The system-wide tunnel needs approval

**Cause.** The fat client's optional system-wide tunnel uses a privileged launchd daemon registered through SMAppService. Its first registration surfaces a background-item approval toggle.
