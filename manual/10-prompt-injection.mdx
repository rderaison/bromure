...
The text shown is **only the offending section**, not the whole scanned content: the source-code detector surfaces the single highest-scoring scan window, and the rules-file detector surfaces the region around the signal that fired (with a little context). A flagged `tool_result` or `CLAUDE.md` can run to many pages; showing just the section that tripped the detector puts the suspect instructions in front of you instead of burying them, capped at 4 KB either way.
...
Prompt-injection detection tries to catch the hijack; [Guardrails](18-glossary.mdx) limits what a hijacked (or merely overenthusiastic) agent can *do*. Guardrails has two layers. The **write policies** described in this section form a per-workspace, host-enforced policy engine that classifies every API call the agent makes to a guarded protocol as a **read**, a **write**, or a **destructive** operation, and applies the workspace's mode for that resource. Beneath them sits the **egress firewall** — an ordered, pf-style ruleset over every connection the VM opens (host, IP/CIDR, protocol, port, and per-HTTP-verb for web traffic), enforced both at the virtual switch and in the proxy, with the VM's port-80/443 flows transparently diverted into the proxy so nothing inside the guest can opt out. The firewall's table editor, rule syntax, and the transparent-interception escape hatch are documented in [Guardrails settings](07-settings/guardrails.mdx#the-egress-firewall). As the pane puts it: Guardrails strips destructive operations from the protocols this agent speaks; it is enforced on the host — inside the proxy — so a misbehaving or compromised agent in the VM can't bypass it, and blocked calls return a hard error the agent sees.
...
| **Outbound connections** (egress firewall) | Guardrails | Ordered allow/deny rule table + **Unmatched traffic** default | No rules; unmatched traffic allowed |
| **Disable transparent interception** | Guardrails | Toggle | Off (interception on) |
