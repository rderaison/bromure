...
## The Security Timeline window

The Trace Inspector answers "what did the agent send?"; the **Security Timeline** (Window menu → **Security Timeline…**) answers "what did Bromure's security engines *decide*?". It is one chronological table — newest first — of every decision the engines made while your workspaces ran:

<p align="center">
  <img src="images/security-timeline.en.jpg" width="700" alt="The Security Timeline window: a chronological table of Firewall, Guardrails, Credential brokering, Prompt injection, Supply chain, and Credential used rows, each colour-coded green for allowed, red for blocked, or blue for informational" />
</p>

| Engine | Example rows |
|---|---|
| **Credential brokering** | `brm_a1b2… → api.openai.com — swapped in sk-oai_…` (blue); `blocked — exfiltration attempt, VM paused` (red) when a fake credential was caught leaving for a host it was never minted for. |
| **Firewall** | `evil.example:443 tcp — blocked` — every egress-firewall verdict. |
| **Supply chain** | `npm left-pad@1.3.0 — blocked — version is 2 days old` and allowed installs alike. |
| **Guardrails** | `DELETE api.github.com/repos/… — blocked` write-policy enforcements. |
| **Prompt injection** | Detector hits with the flagged snippet's preview and the action taken. |
| **Credential used** | SSH signatures and AWS SigV4 signings performed by the host on the agent's behalf. |
| **Upstream TLS** | Upstream certificates the host refused to trust. |

Rows are colour-coded by outcome — green for allowed, red for blocked, blue for informational transformations — and the toolbar filters by engine or free text. The timeline is an in-memory "what just happened" view (capped at 5,000 rows), not an audit store: for durable evidence use the session traces, which outlive the app run and are encrypted at rest.

The engines all run on the host, so when a [remote client](14-remote-access.mdx) mirrors this Mac the timeline mirrors too: the host ships its recent decisions inside the state stream and the client's Security Timeline window shows the same rows.
...
- `egress.firewall` — one per firewall verdict: host, port, protocol, and the action taken.
- `credential.exfiltration` — a fake credential was observed leaving the VM for a host outside its scope (previews and hosts only, never token bytes).
- Detailed HTTP request logs — enrolled installs can additionally stream per-request HTTP metadata from the transparent interception layer, so an organization sees the workspace's outbound HTTP surface, not just its AI traffic.
