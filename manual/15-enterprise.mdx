...
- **The fat client** ([Remote Access](14-remote-access.mdx)) lets one Mac mirror *other* Macs' running instances over SSH, authenticated by SSH host keys and enrolled public keys.

Neither depends on the other. You can enroll without ever using the fat client, mirror hosts without enrolling any of them, or do both. Enrollment's mTLS stack is fleet telemetry only; fat-client mirroring trusts SSH — the two are deliberately not merged in this version.
...
This is distinct from the fat client's notion of a "fleet" — mirroring several *remote hosts* at once over SSH — which is covered in [Remote Access](14-remote-access.mdx#multiple-remotes-fleet-subnet-routing). The two never share a trust root.
