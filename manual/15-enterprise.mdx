...
Enrollment now leads with a browser sign-in. The enrollment sheet (**Bromure Agentic Coding** app-name menu → **Log in to bromure.io…**, directly under **Check for Updates…**) opens on a prominent **Log in to bromure.io** button: clicking it sends you to bromure.io in your browser, you sign in with your account, and the site hands the app its registration through a `bromure://enroll` deep link — no code to paste.

The code-based path still exists for administrator-driven provisioning, collapsed beneath the sign-in button under "If your administrator gave you an enrollment code, enter it here":

1. Get a single-use, six-word enrollment code from your administrator (minted on your user's detail page in bromure.io, scoped to the app `agentic-coding`).
2. Expand the code section and paste it into the **Enrollment Code** field (placeholder: `six-word-enrollment-code`).
3. Optionally edit **Device Name** (it defaults to this Mac's name; help text: "Shown to your administrator so they can recognize this Mac.").
4. Optionally expand **Advanced** and set **Server URL (optional)** for a self-hosted or staging server.
...
> **Note:** A code minted for a different Bromure app is rejected with **Code was issued for app '…', expected 'agentic-coding'.**, and enrolling a second time fails with **Bromure Agentic Coding is already enrolled.**
...
| `egress.firewall` | One event per [egress-firewall](07-settings/guardrails.mdx#the-egress-firewall) verdict: host, port, protocol, and the action taken. |
| `credential.exfiltration` | The compromise detector caught a fake credential leaving the VM for a host outside its scope — credential name, declared and observed hosts, previews only. |
| Detailed HTTP logs | Per-request metadata from the transparent interception layer (method, host, path, status, sizes), giving admins the workspace's outbound HTTP surface beyond its AI traffic. |
