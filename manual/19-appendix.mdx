...
|---|---|
| `profile.json` | Workspace definition, including the `supplyChain` policy block; socket.dev and Depi API keys are stored here host-side and never copied into the VM. Only non-default fields are written. |
...
Two loopback endpoints exist *inside* the guest, not on the Mac: `127.0.0.1:65534` (the in-VM HTTP(S) proxy every shell exports as `HTTPS_PROXY`) and `127.0.0.1:11434` (where the on-host local inference engine appears, via a vsock bridge).
...
|---|---|
| `http_proxy`, `https_proxy`, `HTTPS_PROXY` | `http://127.0.0.1:65534` — the in-VM bridge that carries all guest HTTPS to the host MITM proxy. |
