...
Bromure Agentic Coding is a macOS application that runs AI coding agents — Claude Code, Codex CLI, Grok CLI, Kimi Code, Oh My Pi, and ordinary shell tooling — inside a hardware-virtualized Linux VM on your own Mac, built on Apple's Virtualization.framework. The agent gets a full, persistent Ubuntu development environment; your real credentials never enter it. Everything the agent sends to the outside world passes through a single host-side gateway where isolation, credential protection, supply-chain scanning, prompt-injection detection, and audit tracing are all enforced — a boundary the agent cannot reach around.

This manual documents Bromure Agentic Coding 4.6.0. The executable is `bromure-cli`, the bundle ID is `io.bromure.agentic-coding`, and the app requires macOS 14 (Sonoma) or later on Apple Silicon. Full product details, screenshots, and downloads are published at [bromure.io](https://bromure.io).
...
The same interception point lets you route model calls to inference running on your own Apple Silicon (via MLX) — or to your own vLLM, Ollama, or other OpenAI-compatible server elsewhere on your network — or run hybrid: cloud models normally, with local fallback when the cloud is unreachable. Local turns stream token by token, reasoning models show their thinking live, and vision-capable models accept images. Local models also power the on-device classifiers used elsewhere in the product. See [Local models](13-local-models.mdx).

### Remote access

Your sandbox is reachable from anywhere: through the app itself, from the command line, or over SSH. A second Mac running Bromure Agentic Coding — or an iPhone, iPad, or Apple Vision Pro running the Bromure Remote client — can mirror a remote instance one-to-one — grid, workspaces, tabs, and automations — over an SSH tunnel (**Workspaces** menu → **Connect to Remote Bromure…**). Crucially, security does not weaken when you go remote: consent prompts for credential use, supply-chain bypasses, guardrail writes, and prompt-injection decisions are rendered on the host side of the connection, where a compromised guest can neither see nor forge them, and any timeout or dismissal is treated as a denial. See [Remote access](14-remote-access.mdx).
...
| Proprietary agents | Claude Code, Codex CLI, Grok CLI, Kimi Code, Oh My Pi, and Google Cloud SDK cannot ship inside the redistributable image; they are installed on your machine as signed catalog postinstall steps. Running first-time setup consents to the baseline steps; only steps published in a later update prompt you for approval afterward. |
