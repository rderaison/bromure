...
### Claude, Codex, Grok, and Kimi subscriptions
...
3. An alert fires: *"Bromure detected an outbound attempt to leak a session credential to a host it was not minted for. The VM has been paused."* It offers **Shut down** (the safe default), **Save for Investigation** (export the disk, home, and shared folders for forensics before shutdown), or **Continue** at your own risk. The detection is also recorded as a red **Credential brokering** row in the [Security Timeline](11-tracing.mdx#the-security-timeline-window), and on enrolled installs as a `credential.exfiltration` event.

If the workspace is being [mirrored by a rich client](14-remote-access.mdx), the alert appears on the client as a pending decision prompt — **Shut down** or **Continue** — while only the compromised VM stays paused; the host keeps serving every other workspace, and an unanswered prompt times out to shutdown.
