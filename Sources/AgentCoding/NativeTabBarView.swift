...
// Ported verbatim from Bromure Web (Sources/Browser/NativeTabBarView.swift) so
// the embedded agentic browser uses the exact same Safari-compact tab-pill
// system. The window-titlebar NSToolbar mounting (NativeTabBarChrome /
// CompactBarToolbarDelegate / FlexibleHostingView) is dropped — the AC browser
// hosts NativeCompactBarView directly inside its pane (BrowserPane) instead of
// the window titlebar. Wired to TabBridge by WorkspaceBrowserController.
