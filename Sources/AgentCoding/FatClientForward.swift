...
// The core FatForward socket helpers now live in FatForward.swift (shared).
// These RemoteHost-driven SOCKS/forward methods dial the remote over the
// mac-only ssh forward (RemoteTransport.forwardDial), so they stay here.
#if os(macOS)
extension FatForward {
