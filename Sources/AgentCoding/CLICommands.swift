...
// ControlClient (the HTTP-over-fd control-plane client) and the PTYFrame
// codec moved to ControlClient.swift so the iOS fat client can compile them
// without this file's ArgumentParser CLI surface.
...
enum InteractiveExec {
    ...
    // Internal (not private) for the frame-codec unit tests; forwards to the
    // shared codec (ControlClient.swift) that the iOS pump uses too.
    static func resizePayload(cols: Int, rows: Int) -> [UInt8] {
        PTYFrame.resizePayload(cols: cols, rows: rows)
    }
}
