...
public final class ProfileStore {
    ...
    /// ~/.bromure-tab-agent.sh: runs inside the VM as the X session foreground
    /// process. Launches one fullscreen kitty attached to a single tmux session
    /// ("bromure") whose WINDOWS are the tabs, publishes the window list to
    /// /mnt/bromure-outbox/tabs.txt, and dispatches host commands from
    /// cmd-*.txt (one line each):
    ///     new-tab            → tmux new-window
    ///     select-tab <index> → tmux select-window
    ///     close-tab <index>  → tmux kill-window
    ...
    # fullscreen kitty is attached as the GUI client; `cm attach` adds a CLI
    # client to the same session (lockstep). tmux is the sole source of truth
    # for which tabs exist — no per-process roster / liveness tracking.
    TMUX_S="bromure"
