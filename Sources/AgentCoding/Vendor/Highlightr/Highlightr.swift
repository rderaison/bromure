...
//
//
//  Vendored from https://github.com/raspu/Highlightr (MIT — see LICENSE in
//  this directory). The upstream SPM package loads highlight.min.js and its
//  theme CSS via `Bundle.module`, whose swift-build accessor only checks the
//  .app ROOT and the absolute dev build path — it traps on any relocated
//  install. Vendoring lets the resources ride the app's own resource bundle
//  (`acResourceBundle`, "highlightr/" subdirectory), the same pattern every
//  other bromure-ac resource uses. Only the bundle resolution and the
//  default theme were changed; the highlighting engine is untouched.
...
open class Highlightr
{
    ...
    public init?(highlightPath: String? = nil)
    {
        ...
        let bundle = acResourceBundle
        self.bundle = bundle
        guard let hgPath = highlightPath ?? bundle.path(forResource: "highlightr/highlight.min", ofType: "js") else
        {
            return nil
        }
        ...
        // bromure-ac ships only the themes it uses; default to one of them.
    ...
    {
        guard let defTheme = bundle.path(forResource: "highlightr/"+name+".min", ofType: "css") else
        {
            return false
        }
    ...
    {
        let paths = bundle.paths(forResourcesOfType: "css", inDirectory: "highlightr") as [NSString]
        var result = [String]()
        for path in paths {
