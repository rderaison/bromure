...
final class RemoteHostController {
    ...
    func newTab(_ id: Profile.ID) {
        // No optimistic echo here: predicting the new window index and mounting a
        // surface for it made the terminal attach BEFORE the guest created the
        // tmux window ("can't find window N" for ~10s until it caught up). The
        // push model reflects the real new tab in ≤0.5s (or ~1 poll without push),
        // by which point the window exists, so the attach is clean.
...
final class RemoteHostWindow: NSWindow {
    ...
    private var sessionStageActions: SessionStageActions {
        SessionStageActions(
            ...
            editMachine: { [weak self] pid in self?.openWorkspaceSettings(pid) },
