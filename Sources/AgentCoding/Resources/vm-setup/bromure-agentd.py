...
def task_install_ca():
    ...
    # Java has no bundle-file env var — import into the default JDK cacerts
    # (Java 9+ `-cacerts`). Best-effort; drop any prior alias first so a re-run
    # doesn't fail on "already exists".
    if shutil.which("keytool"):
        _sudo(["keytool", "-delete", "-cacerts", "-storepass", "changeit",
               "-alias", "bromure-ca"])
        rc = _sudo(["keytool", "-importcert", "-cacerts", "-storepass", "changeit",
                    "-noprompt", "-trustcacerts", "-alias", "bromure-ca", "-file", pem])
        if rc.returncode == 0:
            log("session", "installed MITM CA into Java cacerts")
