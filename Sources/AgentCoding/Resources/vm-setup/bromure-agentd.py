...
def _sudo_write(path, content, mode="0644"):
    """Write `content` to `path` as root (tee), then chmod. Returns success."""
    try:
        subprocess.run(["sudo", "tee", path], input=content, text=True,
                       stdout=_DEVNULL, stderr=_DEVNULL, check=True)
        _sudo(["chmod", mode, path])
        return True
    except Exception:
        return False


# CA-trust env delivered to processes that never source the interactive
# proxy.env. Value paths exist after `update-ca-certificates` + the raw-PEM
# install in task_install_ca.
_CA_TRUST_ENV = {
    "NODE_EXTRA_CA_CERTS": "/etc/ssl/certs/bromure-ca.pem",
    "REQUESTS_CA_BUNDLE": "/etc/ssl/certs/ca-certificates.crt",
    "SSL_CERT_FILE": "/etc/ssl/certs/ca-certificates.crt",
...
def _merge_etc_environment(env):
    """Idempotently write a marked KEY=VALUE block into /etc/environment (read by
    PAM for non-interactive/login sessions: ssh host cmd, cron). Replaces any
    prior Bromure block so re-runs never duplicate."""
    begin, end = "# >>> bromure ca env >>>", "# <<< bromure ca env <<<"
    cur = ""
    try:
    ...
    if begin in cur and end in cur:
        cur = cur.split(begin)[0].rstrip("\n") + "\n" + cur.split(end, 1)[1].lstrip("\n")
    block = "\n".join(f"{k}={v}" for k, v in env.items())
    new = cur.rstrip("\n") + f"\n{begin}\n{block}\n{end}\n"
    _sudo_write("/etc/environment", new.lstrip("\n"))


# Idempotent sync that folds the MITM CA into Java's trust store. Runs at
# session setup AND from an apt DPkg hook, so it can be installed while no JVM
# exists and "wait": the moment a Java app pulls in a JRE/JDK, the hook re-runs
# and the CA lands in Java's keystore — no session restart, no baked-in Java.
#
# Why this shape (learned the hard way):
#  - Java's PKCS12 KeyStore silently DROPS openssl's plain cert bags (they lack
#    the Oracle trusted-cert bag attribute), so an openssl-built truststore
#    loads as 0 entries — useless. keytool is the only reliable way to write a
#    Java truststore, and it needs a JVM. Hence: wait for Java, then keytool.
#  - `update-ca-certificates` folds our CA into the shared java keystore only
#    the FIRST time its ca-certificates-java hook sees the cert; it won't
#    re-add after a delete, so it's not a reliable heal. keytool -importcert is.
_JAVA_CA_SYNC_PATH = "/usr/local/sbin/bromure-java-ca-sync"
_JAVA_CA_SYNC = r"""#!/bin/sh
# Generated by Bromure AC. Fold the MITM CA into Java's trust store. No-op until
# a JVM (keytool) is installed. Idempotent.
command -v keytool >/dev/null 2>&1 || exit 0
pem=/etc/ssl/certs/bromure-ca.pem
[ -r "$pem" ] || exit 0

# 1. Shared system Java keystore: apt/.deb JREs & JDKs read it through the
#    ca-certificates-java symlink at $JAVA_HOME/lib/security/cacerts.
if ! keytool -list -cacerts -storepass changeit -alias bromure-ca >/dev/null 2>&1; then
    keytool -importcert -cacerts -storepass changeit -noprompt -trustcacerts \
        -alias bromure-ca -file "$pem" >/dev/null 2>&1 || true
fi

# 2. A Java app that ships its OWN cacerts (bundled JRE / downloaded JDK) can't
#    be edited in place — point it at the shared keystore (now carrying our CA
#    plus the public roots) via JAVA_TOOL_OPTIONS. Only once that keystore
#    exists, so no JVM is ever sent at a missing trustStore.
shared=/etc/ssl/certs/java/cacerts
dropin=/etc/profile.d/bromure-java.sh
if [ -r "$shared" ]; then
    cat > "$dropin" <<EOF
# Generated by Bromure AC.
export JAVA_TOOL_OPTIONS="-Djavax.net.ssl.trustStore=$shared -Djavax.net.ssl.trustStorePassword=changeit"
EOF
fi
exit 0
"""

_JAVA_CA_APT_HOOK_PATH = "/etc/apt/apt.conf.d/99-bromure-java-ca"
_JAVA_CA_APT_HOOK = (
    "// Generated by Bromure AC: re-fold the MITM CA into Java's trust store\n"
    "// after any package op, so installing a JRE/JDK trusts the proxy at once.\n"
    'DPkg::Post-Invoke { "' + _JAVA_CA_SYNC_PATH + ' >/dev/null 2>&1 || true"; };\n'
)
...
def task_install_ca():
    ...
    # Deliver the CA-trust env beyond the interactive shell: /etc/profile.d for
    # login shells, /etc/environment (PAM) for ssh-cmd and cron. systemd system
    # services read neither — dockerd gets its own drop-in; other units are rare
    # in an agent VM.
    profile_d = "# Generated by Bromure AC.\n" + "".join(
        f'export {k}="{v}"\n' for k, v in _CA_TRUST_ENV.items())
    _sudo_write("/etc/profile.d/bromure-ca.sh", profile_d)
    _merge_etc_environment(_CA_TRUST_ENV)

    # Java trust — install the sync script + apt hook so a JRE added LATER still
    # trusts the CA, then run it once for a JVM that's already here (base image
    # or resumed session).
    _sudo_write(_JAVA_CA_SYNC_PATH, _JAVA_CA_SYNC, mode="0755")
    _sudo_write(_JAVA_CA_APT_HOOK_PATH, _JAVA_CA_APT_HOOK)
    if _sudo([_JAVA_CA_SYNC_PATH]).returncode == 0 and shutil.which("keytool"):
        log("session", "synced MITM CA into Java trust store")
