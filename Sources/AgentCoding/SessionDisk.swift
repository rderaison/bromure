...
public final class SessionDisk {
    ...
    /// on-host engine. `wire_api = "chat"` makes Codex use the OpenAI
    /// chat-completions endpoint (which vllm-mlx serves) rather than its
    /// default Responses API. Top-level keys come first so it stays valid
    /// TOML when the MCP `[mcp_servers.*]` tables are appended after it.
    static func codexLocalProviderTOML(model: String) -> String {
        """
