...
public final class SessionDisk {
    ...
    public func prepareMetadataShare(forRestore: Bool = false) throws -> URL {
        ...
        if !enabledMCP.isEmpty {
            ...
            // Claude Code format
            let claudeJSON = Self.claudeCodeMCPConfig(servers: enabledMCP)
            ...
            // Codex format
            let codexTOML = Self.codexMCPConfig(servers: enabledMCP)
            try codexTOML.write(
    ...
    static func claudeCodeMCPConfig(servers: [MCPServer]) -> String {
        ...
        for server in servers {
            ...
            if !env.isEmpty {
                entry["env"] = env.reduce(into: [String: String]()) {
                    $0[$1.name] = $1.value
                }
            }
    ...
    static func codexMCPConfig(servers: [MCPServer]) -> String {
        var lines: [String] = ["# Generated by Bromure AC; do not edit."]
        for server in servers {
            lines.append("")
            ...
            if !env.isEmpty {
                ...
                for v in env {
                    lines.append("\(v.name) = \(tomlQuote(v.value))")
                }
            }
