...
public final class SessionDisk {
    ...
    static func codexLocalProviderTOML(model: String, contextWindow: Int) -> String {
        // Pin the model metadata. Codex can't look up the `bromure-local` sentinel
        // in its model registry, so without this it warns ("metadata not found")
        // and falls back to conservative defaults — including a low max-output cap
        // that truncates large tool calls (e.g. writing a multi-KB file). Track the
        // workspace's active local model: ~1/4 of context for output, capped at 32k.
        let maxOutput = min(max(contextWindow / 4, 4096), 32_768)
        return """
