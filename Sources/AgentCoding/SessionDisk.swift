...
public final class SessionDisk {
    ...
    public func prepareMetadataShare(forRestore: Bool = false) throws -> URL {
        ...
        if let assets = mitmAssets {
            ...
            // Route the guest through the host MITM — UNLESS the profile opted
            // into full passthrough (Guardrails → "Disable transparent
            // interception"). With interception off, the transparent divert at
            // the switch is already skipped; dropping these env vars too means
            // proxy-aware tools (apt/git/pip/curl/node/…) go straight out as
            // well, so a certificate-pinned client sees the real upstream cert
            // instead of the proxy's re-signed leaf. The CA-bundle vars below
            // stay (they just point at the system roots), so ordinary TLS still
            // validates normally.
            if !profile.disableTransparentProxy {
                proxyLines += [
        ...
        if let ompSpec = profile.allToolSpecs.first(where: { $0.tool == .omp }) {
            ...
            // wrapper passes --config): the built-in puppeteer browser tool
