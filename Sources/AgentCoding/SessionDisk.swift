...
public final class SessionDisk {
    ...
    /// custom engine (a first boot against an unprobed server does one
    /// 1.5s-capped synchronous probe; the launch-path probe heals the cache
    /// after that). 128k when nothing better is known.
    static func localModelContext(profile: Profile) -> Int {
        ...
        if let base = profile.localEngineBaseURL {
            if let n = EngineModelMeta.contextLength(base: base, model: id) { return n }
            if let n = EngineModelMeta.refresh(base: base, apiKey: profile.localEngineAPIKey,
                                               model: id, waitUpTo: 1.5) { return n }
            return 128_000
        }
