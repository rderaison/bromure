...
DNSMASQ_UPSTREAM_BACKUP = "/tmp/bromure/upstream.conf.wg-backup"
...
def apply_vpn_dns(dns_servers):
    ...
    rewrite dnsmasq's upstream file and send it SIGHUP.  dnsmasq re-reads
    its resolv-file on SIGHUP (and flushes the cache) without dropping
    anything, unlike squid in foreground (-N) mode which loses connectivity
    on reconfiguration.  Note it does NOT re-read server= lines from its
    config file on SIGHUP — this used to rewrite pihole.conf, a silent no-op
    that left every lookup on the pre-tunnel resolver.
    ...
    # Swap dnsmasq's upstream file and SIGHUP (squid always uses dnsmasq).
    # The backup is taken once so a repeated enable can't capture VPN DNS as
    # the "original" and leave it behind on teardown.
    try:
        ...
        with open(DNSMASQ_UPSTREAM, "w") as f:
            f.write("# VPN DNS (wireguard-agent); pre-tunnel upstreams restored on disable\n")
            for srv in dns_servers:
                f.write(f"nameserver {srv}\n")
