#!/usr/bin/python3 -u
"""Bromure corporate-guard agent — runs inside the guest VM.

Bridges Chrome native messaging (stdin/stdout) to the host over vsock.
The corporate-guard Chrome extension sends "open this URL in a private
Bromure profile" requests; this agent forwards them to the host
CorporateGuardBridge which decides which profile to use (prefer an
ephemeral "Private Browsing" profile, new session or existing).

Protocol: newline-delimited JSON over vsock (port 5310).
Native messaging: 4-byte LE length prefix + JSON (Chrome standard).

Spawned on demand by Chromium when the extension calls
sendNativeMessage / connectNative to com.bromure.corporate_guard.
"""
