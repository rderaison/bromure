...
# The shared macOS strings catalogs — the same <lang>.lproj the AC app ships.
LOCALIZATION_SRC = os.path.join(REPO, "Sources", "AgentCoding")
...
def main():
    ...
    settings_ref = uid("fileRef:App/Settings.bundle")
    settings_build = uid("buildFile:App/Settings.bundle")

    # Localizations. The shared UI compiles straight into the app, so its
    # Text("…") lookups resolve against Bundle.main — which means the app
    # bundle needs the same <lang>.lproj/Localizable.strings the macOS app
    # gets (build.sh copies them into Contents/Resources there). Without
    # this the iOS/iPadOS app renders raw English keys in every locale.
    # Modeled as a PBXVariantGroup, which is how Xcode itself represents a
    # localized resource: one child file reference per language, `name` =
    # the locale, and the build phase references the GROUP.
    loc_locales = sorted(
        d[: -len(".lproj")]
        for d in os.listdir(LOCALIZATION_SRC)
        if d.endswith(".lproj")
        and os.path.isfile(os.path.join(LOCALIZATION_SRC, d, "Localizable.strings")))
    loc_group = uid("variantGroup:Localizable.strings")
    loc_build = uid("buildFile:Localizable.strings")
    loc_refs = {lang: uid("fileRef:lproj:" + lang) for lang in loc_locales}
    ...
    L.append(f"\t\t{settings_build} /* Settings.bundle in Resources */ = {{isa = PBXBuildFile; fileRef = {settings_ref} /* Settings.bundle */; }};")
    L.append(f"\t\t{loc_build} /* Localizable.strings in Resources */ = {{isa = PBXBuildFile; fileRef = {loc_group} /* Localizable.strings */; }};")
    ...
    L.append(f'\t\t{settings_ref} /* Settings.bundle */ = {{isa = PBXFileReference; lastKnownFileType = "wrapper.plug-in"; path = "App/Settings.bundle"; sourceTree = "<group>"; }};')
    for lang in loc_locales:
        rel = os.path.relpath(
            os.path.join(LOCALIZATION_SRC, lang + ".lproj", "Localizable.strings"), PKG)
        L.append(f'\t\t{loc_refs[lang]} /* {lang} */ = {{isa = PBXFileReference; '
    ...
    L.append(f"\t\t\tfiles = (\n\t\t\t\t{assets_build} /* Assets.xcassets in Resources */,"
             ...
             f"\n\t\t\t\t{loc_build} /* Localizable.strings in Resources */,\n\t\t\t);")
    L.append("\t\t\trunOnlyForDeploymentPostprocessing = 0;")
    L.append("\t\t};")
    ...
    # PBXVariantGroup — the localized Localizable.strings. Xcode expands the
    # group's children into <lang>.lproj/Localizable.strings inside the built
    # app, which is where Bundle.main looks them up.
    L.append("\n/* Begin PBXVariantGroup section */")
    L.append(f"\t\t{loc_group} /* Localizable.strings */ = {{")
    L.append("\t\t\tisa = PBXVariantGroup;")
    L.append("\t\t\tchildren = (")
    for lang in loc_locales:
        L.append(f"\t\t\t\t{loc_refs[lang]} /* {lang} */,")
    L.append("\t\t\t);")
    L.append("\t\t\tname = Localizable.strings;")
    L.append("\t\t\tsourceTree = \"<group>\";")
    L.append("\t\t};")
    L.append("/* End PBXVariantGroup section */")
